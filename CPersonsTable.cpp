...
#define SELECT_BY_NAME _T("SELECT * FROM PERSONS WHERE (UPPER([FIRST_NAME]) LIKE UPPER(N'%%%s%%') AND '%s' != '' ) OR \
                           ...
                           (UPPER([CITIZEN_ID]) LIKE UPPER(N'%%%s%%') AND '%s' != '' ) OR \
...
CPersonsTable::CPersonsTable() : CGenericTable(m_recPerson, PERSONS_TABLE_NAME)
{
}

CPersonsTable::CPersonsTable(CSession& oSession) :
    CGenericTable(m_recPerson, PERSONS_TABLE_NAME, oSession)
{
}
